...
local Aether = loadstring(game:HttpGet("https://raw.githubusercontent.com/hoctienganhcungtruong/Aether/main/v11.lua"))()

-- Instantiate UI Window (Height reduced 20% from 420 to 336)
local UI = Aether.new(
...
-- FPS Booster consolidated state tracking
...
MainTab:AddButton("Unlock All Islands", function()
    task.spawn(function()
        for _, island in ipairs(islandData) do
            ...
            task.wait(0.1) -- Reduced teleport time delay to 0.1 seconds per island
        end
    end)
end)
...
-- Merged Optimization Button
SettingsTab:AddButton("FPS Booster", function()
    fpsBoosterEnabled = not fpsBoosterEnabled
    
    -- Update respective runtime states
    removeEffectsEnabled = fpsBoosterEnabled
    muteSoundsEnabled = fpsBoosterEnabled
    removeTexturesEnabled = fpsBoosterEnabled
    
    -- 1. Manage Effects (Trails, Particles)
    ...
    for _, obj in ipairs(game:GetDescendants()) do
        if obj:IsA("Sound") then
            obj.Volume = fpsBoosterEnabled and 0 or (obj:FindFirstChild("OriginalVolume") and obj.OriginalVolume.Value or obj.Volume)
        end
    end
    ...
    -- 4. Manage 3D Rendering (Black Screen CPU Saver)
    game:GetService("RunService"):Set3DRenderingEnabled(not fpsBoosterEnabled)
end)
